...
def get_devices():
    if request.method == "GET":
        devices = yaml_function("./APIs/data/devices.yml", "read")
...
def get_policies():
    ...
    if request.method == "GET":
        policies = yaml_function("./APIs/data/policies.yml", "read")
        ...
        data.append(new_data)
        yaml_function("./APIs/data/policies.yml", "write", data=data)
        return jsonify({"data": request.json}), 201
...
def delete_policy(id: str):
    ...
    for policy in policies:
        if policy.get("id") == id:
            ...
            yaml_function("./APIs//data/policies.yml", "write", data=policies)
            return jsonify({}), 204
